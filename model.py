...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=32, patch_size=4, in_chans=48, embed_dim=128):
        super().__init__()
...
class GFNetPyramid(nn.Module):
    ...
    def __init__(self, img_size=8, band =48, patch_size=4,  in_chans=48, num_classes=7, embed_dim=[128, 256], depth=[2, 2], n_heads=8, d_head=32,
        ...
        # sizes = [16, 8]
        for i in range(2):
            sizes[i] = sizes[i] * img_size // 8
            # sizes[i] = sizes[i] * img_size // 16
            # sizes[i] = sizes[i] * img_size // 32
            # sizes[i] = sizes[i] * img_size // 64
    ...
    def forward_features(self, x):
        ...
        for i in range(2):
            ...
            x_aug = self.patch_embed[i](x_aug)
            if i == 0:
                x = x + self.pos_embed
                x_aug = x_aug + self.pos_embed

            for blk in self.blocks[i]:
                if isinstance(blk, BlockLayerScale):
                    blk.enable_spectrum_noise(True)
            x_aug = self.blocks[i]((x_aug))
...
class DSPLTnet(nn.Module):
    ...
    def __init__(self, model, NotearsClassifier, num_classes, hparams):
        ...
        self.loss_multi_proto_con = MultiDomainPrototypePLoss(num_classes, 2, hparams['temperature']) 
    ...
    def update(self, x, y, **kwargs):
        ...
        normalized_masked_f = F.normalize(masked_f, dim=1)
        normalized_masked_f_aug = F.normalize(masked_f_aug, dim=1)
        
        for single_masked_f, label_y in zip( normalized_masked_f_aug, y):
        ...
        proto_rec, masked_proto = self.dag_mlp(
            x=prototypes_y.view(self.num_classes, -1),
            y=self.prototypes_label)

        # reconstruction loss
        loss_rec = F.cosine_embedding_loss(
            proto_rec, 
            prototypes_y.view(self.num_classes, -1),
        ...
        loss_contr = self.hparams["weight_con"] * (self.loss_proto_con(masked_f_aug, prototypes_y, y) + self.supcon(z, y)) #+ self.supcon(z, y)
        ...
        if kwargs['step'] >= self.hparams["dag_anneal_steps"]:
            loss = loss_inv_ce + loss_dag + loss_contr
        else:
            loss = loss_inv_ce + loss_contr 
        
        loss = loss_inv_ce + 0.1*loss_dag + loss_contr
        loss = F.cross_entropy(self.inv_classifier(f), y)
        loss = loss_inv_ce + loss_contr 
    ...
    def predict(self, x):
        f_aug, f = self.featurizer(x)
        masked_f = self.dag_mlp(f)
        return self.inv_classifier(masked_f)#, masked_f
