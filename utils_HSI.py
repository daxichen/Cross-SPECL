...
def test_metrics(prediction, target, ignored_labels=[], n_classes=None):
    ...
    prediction = prediction[ignored_mask]-1
...
def test(net, diffusion, img, hyperparams):
    """
    ...
    clip_denoised = hyperparams['clip_denoised']
    ...
    probs_sample = np.zeros(img.shape[:2] + (n_classes,))
    ...
    for batch in tqdm(
        ...
        model_kwargs = {"ref_img" : data}

        pred_xstart, _ = diffusion.p_sample_loop(
            net,
            (cur_batch_size , 1, N_BANDS, patch_size, patch_size),
        ...
        with torch.no_grad():
            ...
            time = torch.zeros([data.shape[0]], dtype=torch.long).to(gpu)
            output = net(data, time, cls_mode=True)
            sample = pred_xstart
            pred_x_t = net(sample, time, cls_mode=True)
            if isinstance(output, tuple):
                output = output[0]
                pred_x_t = pred_x_t[0]
            output = output.to("cpu")
            pred_x_t = pred_x_t.to("cpu")

            output = output.numpy()
            pred_x_t = pred_x_t.numpy()
